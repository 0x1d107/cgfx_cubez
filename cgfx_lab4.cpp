...
struct Shape3d{
    ...
    static Shape3d prism(int n,double a,double h){
        std::vector<sf::Vector3f> V(2*n);
        std::vector<std::pair<int,int>> E(3*n);
        double r = a/sin(3.1415/n);
        for(int i=0;i<n;i++){
            V[2*i]=(sf::Vector3f(r*cos(i*2*3.1415/n),r*sin(i*2*3.1415/n),-h/2));
            V[2*i+1]=(sf::Vector3f(r*cos(i*2*3.1415/n),r*sin(i*2*3.1415/n),h/2));
            E[3*i] = std::make_pair(2*i,2*i+1);
            E[3*i+1] = std::make_pair(mod(2*i+2,2*n),2*i);
            E[3*i+2] = std::make_pair(mod(2*i+3,2*n),mod(2*i+1,2*n));
        }
        return Shape3d{V,E};
    }
};
...
int main (){
    ...
    Shape3d p1 = Shape3d::prism(5,50,200).transform(Matrix::translate(200,0,0));

    Shape3d p2 = Shape3d::prism(5,50,200).transform(Matrix::translate(-200,0,0)*Matrix::rotateY(3.1415/2));
    Matrix projection = PERSPECTIVE;

    sf::RenderWindow win(sf::VideoMode(800, 600), "CGFX lab4");
    ...
    while(win.isOpen()){
        ...
        p1.draw(win,projection);
        p2.draw(win,projection);
        win.display();
    }
} 
